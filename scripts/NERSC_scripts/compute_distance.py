...
for i,j in worker_tasks[rank]:
    S1 = pd.read_csv(os.path.join(tmp_dir,'proc_%d_S.csv'%i),index_col=0)
    S2 = pd.read_csv(os.path.join(tmp_dir,'proc_%d_S.csv'%j),index_col=0)
    dist = abs(np.dot(S1.T,S2))
    dist[dist < .5] = 0
    sparse.save_npz(os.path.join(tmp_dir,'dist_%d_%d.npz'%(i,j)),sparse.coo_matrix(dist))
    counter += 1
